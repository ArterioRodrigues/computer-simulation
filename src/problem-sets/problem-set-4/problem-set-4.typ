...
*a)* ${X_n}$ *is not a Markov Chain*. Because the current state depends on the past.  
In the beginning:
...
$lim_(n arrow infinity) PP(X_n = i)$ = $PP("Heads") * PP(X_n = i | "Heads") + PP("Tails") * PP(X_n = i | "Tails")$ \ 
$lim_(n arrow infinity) PP(X_n = i)$ = $p pi_i^(1) + (1 - p) pi_i^(2)$

\

*b)* ${X_n}$ *is a Markov Chain*. Each state is based on a random probability, so the past doesn't influence the future.  
The one-step transition probabilities depend only on the current state.
...
$
Since $pi_2 = 1 - pi_1$, we can solve the first equation.  
Take $P^(1)$ as an example:

$pi_1 = 0.9pi_1 + 0.2pi_2$ \ 
$pi_1 = 0.9pi_1 + 0.2(1 - pi_1)$ \ 
$pi_1 = 0.9pi_1 + 0.2 - 0.2pi_1$ \ 
$0.3pi_1 = 0.2$ \ 
$pi_1 = 2/3$ \ 
so $pi_2 = 1 - pi_1 = 1/3$.
...
$pi_1 = 0.5pi_1 + 0.1pi_2$ \ 
$pi_1 = 0.5pi_1 + 0.1(1 - pi_1)$ \ 
$pi_1 = 0.5pi_1 + 0.1 - 0.1pi_1$ \ 
$0.6pi_1 = 0.1$ \ 
$pi_1 = 1/6$ \ 
so $pi_2 = 1 - pi_1 = 5/6$.
...
*a)* Let $T(j)$ be the expected number of steps (iterations) until success when the chain starts in state $j$.  
At each state $j$:

- With probability $1/j$, the algorithm succeeds on the next step (and stops).  
- Otherwise (with probability $(j - 1)/j$), it moves to some state $k \in \{1, dots, j - 1\}$ with
  $\Pr(K = k) = 2k / j^2$ (and continues).

The first-step equation is:

$T(j) = 1 + sum_(k = 1)^(j - 1) (2k / j^2) * T(k), quad T(1) = 1.$

We show that $T(m) = O(ln m)$.

Assume for all $k < j$ that $T(k) <= a ln k + b$ for some constants $a, b > 0$.  
Then:

$T(j) = 1 + sum_(k = 1)^(j - 1) (2k / j^2)(a ln k + b)$ \
$T(j) = 1 + (2a / j^2) sum_(k = 1)^(j - 1) k ln k + (2b / j^2) sum_(k = 1)^(j - 1) k.$

Use the integral bound $sum_(k = 1)^(j - 1) k ln k <= integral_0^j x ln x "dx"$  
Compute the integral:

$integral_0^j x ln x "dx" = (j^2 / 2) ln j - (j^2 / 4).$  

Also, $sum_(k = 1)^(j - 1) k = j(j - 1)/2 <= j^2 / 2.$  
Substitute these bounds:

$T(j) <= 1 + (2a / j^2) * ((j^2 / 2) ln j - j^2 / 4) + (2b / j^2)(j^2 / 2).$ \
$T(j) <= 1 + a ln j - a/2 + b = a ln j + (1 - a/2 + b).$

Choose $a >= 2$ so that $(1 - a/2) <=0$. For instance, take $a = 2$.  
Then $T(j) <= 2 ln j + b.$

Pick $b$ large enough to cover base cases (e.g., $b = 1$ works since $T(1) = 1 <= 2 ln 1 + 1$).  
By induction, the bound holds for all $j$.

Therefore, 

$T(m) <= 2 ln m + 1 = O(ln m).$

The expected number of iterations until success when starting at $m$ grows at most on the order of $ln m$.
...
#image("result.png")
The algorithm operates as a Markov chain where from state $j$, we transition to state 0 (success) with probability $1/j$, or to state $k in {1, 2, ..., j-1}$ with probability $2k/j^2$. Starting from state $m$, we simulate until absorption at state 0.

The simulation uses the *inverse transform method* for sampling. For each step:
1. Generate $u tilde "Uniform"(0,1)$
2. If $u < 1/j$, set state = 0 (found zero)
3. Otherwise, sample state $k$ by finding where $u$ falls in the cumulative distribution: $sum_(i=1)^k 2i/j^2$
...
*Key Observations:*
- All relative errors achieve $lt.eq 5%$ target using adaptive sample sizes (189-294 samples)
- The simulated mean follows logarithmic growth as predicted by part (a)
- The ratio simulated/theoretical is approximately constant ($approx 0.78$), indicating $bb(E)[T_m] approx 1.6 dot ln(m)$


The simulation confirms the $O(ln(m))$ bound from part (a) through multiple checks:

1. *Growth Rate Test:* When $m$ doubles from 50 to 100, iterations increase by $7.35 - 6.48 = 0.87$, compared to theoretical increase of $2 ln(2) approx 1.39$ (ratio $approx 0.63$)

2. *Log-Scale Linearity:* Plotting mean iterations vs. $ln(m)$ shows a clear linear relationship with slope $approx 1.6$

3. *Stability of Proportionality:* The ratio of simulated to theoretical values remains stable across all test cases, confirming consistent logarithmic behavior

The empirical constant factor is *1.6*, giving the precise formula: $bb(E)[T_m] approx 1.6 ln(m)$. This represents the expected number of iterations for the zero-finding algorithm starting from state $m$.

*Code Strategy:* The implementation uses modular functions for simulation, adaptive sample sizing, and statistical estimation. Numerical stability is ensured through cumulative probability calculations, and the Central Limit Theorem justifies the confidence interval construction for $n gt.eq 30$.

#pagebreak()
== Problem 3: Viral Infection Model

Let $N$ be the total population, and $X(t) in {0, 1, dots, N}$ be the number of infected individuals at time $t$. Contacts between any pair of the $binom(N, 2)$ individuals occur according to a Poisson process of rate $Lambda$. If a contact is between an infected and a healthy individual, the healthy one becomes infected with probability $p$.


*a)* The process ${X(t), t >= 0}$ is a *Continuous-Time Markov Chain (CTMC)*. Since the state only changes by $plus.minus 1$ (infection or recovery), it is a *Birth and Death Process*.

=== General Transition Rates

The rate of a contact between one infected and one healthy individual is $Lambda dot (i(N-i)) / binom(N, 2)$.
The *Birth Rate* ($lambda_i$, transition $i -> i+1$, infection) is:
...
*b)* Let $E_i$ be the expected time until absorption starting from state $i$.
...
*c)* A simulation is performed to estimate the time until absorption for the process of Model 2, starting at $X(0)=1$.
...
def simulate_time_to_absorption(N: int, Lambda: float, p: float, gamma: float) -> float:
    ...
    while X > 0:
        ...
        # Assuming np.random.exponential(scale=1.0) gives Exp(rate=1)
...
*d)* The model is modified for the scenario where a recovered individual has life immunity. The contagion follows the same model as before.
...
We do not know the population variance, so we use Student's $t$-distribution to account for estimation uncertainty. This is the standard sequential fixed-width stopping procedure.
...
Since we assume we don't know the mean and variance of the service distribution, the program reads consecutive service times (treating them as data) rather than using the theoretical $Gamma(3,4)$ parameters.
...
*(b)* Proof that $N$ is a Stopping Time
...
$N$ is a stopping time with respect to the filtration ${cal(F)_n}$.
...
(c) Empirically estimate the coverage probability of the 95% CI produced by the sequential fixed-width algorithm. Over $M = 20$ independent experiments of the whole sequential procedure, compute how often the final CI contains the true $theta$.
